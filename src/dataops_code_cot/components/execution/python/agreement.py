...
class DataManager:
    def __init__(
        self, dual_exec_results, sampled_code_by_task, sampled_test_case_by_task, limit
    ):
    ...
    def _get_solution_frequency(self):
        ...
        # print(self.solution_frequency_by_task)
    ...
    def _get_test_case_frequency(self):
        ...
        # print(self.test_case_frequency_by_task)
        ...
        for result in self.dual_exec_results:
            ...
            for idx, test_case in enumerate(result["test_cases"]):
                ...
                if test_case not in self.test_case_frequency_by_task[result["task_id"]]:
                    continue
                self.passed_solution_test_case_pairs_by_task[result["task_id"]].add(
                    (result["completion"], test_case)
                )
                self.passed_unique_solutions_by_task[result["task_id"]].add(
                    result["completion"]
                )
    ...
    def _get_solution_and_test_case_ids(self):
        for task_id in self.solution_frequency_by_task.keys():
            self.solution_string_to_id_range_by_task[task_id] = (
                self._build_string_to_id_range(
                    self.solution_frequency_by_task[task_id],
                    self.passed_unique_solutions_by_task[task_id],
                )
            )
            self.test_case_string_to_id_range_by_task[task_id] = (
                self._build_string_to_id_range(
                    self.test_case_frequency_by_task[task_id],
                    self.passed_unique_test_cases_by_task[task_id],
                )
            )
    ...
    def _get_expanded_dual_exec_result(self):
        for task_id in self.passed_solution_test_case_pairs_by_task.keys():
            for (
                solution_str,
                test_case_str,
            ) in self.passed_solution_test_case_pairs_by_task[task_id]:
                solution_id_range = self.solution_string_to_id_range_by_task[task_id][
                    solution_str
                ]
                test_case_id_range = self.test_case_string_to_id_range_by_task[task_id][
                    test_case_str
                ]
                self.expanded_passed_solution_test_case_pairs_by_task[task_id] += (
                    self._get_expanded_by_id_range(
...
class DualAgreement:
    def __init__(self, data_manager):
        ...
        self.dual_exec_results_by_task = (
            data_manager.expanded_passed_solution_test_case_pairs_by_task
        )
        ...
        for task_id in self.dual_exec_results_by_task:
            for solution, test_case in self.dual_exec_results_by_task[task_id]:
                if solution in self.solution_passed_cases_by_task[task_id]:
                    self.solution_passed_cases_by_task[task_id][solution].append(
                        test_case
                    )
                else:
                    self.solution_passed_cases_by_task[task_id][solution] = [test_case]

        # print(self.solution_passed_cases_by_task)
    ...
    def _get_caseset_passed_solutions(self):
        for task_id in self.solution_passed_cases_by_task.keys():
            for solution in self.solution_passed_cases_by_task[task_id].keys():
                case_set = tuple(
                    sorted(self.solution_passed_cases_by_task[task_id][solution])
                )  # case_set: set of (test_case, score)
                if case_set in self.caseset_passed_solutions_by_task[task_id]:
                    self.caseset_passed_solutions_by_task[task_id][case_set].append(
                        solution
                    )
                else:
                    self.caseset_passed_solutions_by_task[task_id][case_set] = [
                        solution
                    ]
        # print(self.caseset_passed_solutions_by_task)

    def get_sorted_solutions_without_iter_v1(self):
        ...
        for task_id in self.caseset_passed_solutions_by_task.keys():
            ...
            for case_set in self.caseset_passed_solutions_by_task[task_id].keys():
                ...
                solution_str_set = [
                    self.solution_id_to_string_by_task[task_id][solution]
                    for solution in solution_set
                ]
                flatted_case_set_passed_solutions.append(
                    (solution_str_set, case_set_score * solution_set_score)
                )

                # print(task_id, case_set_score,solution_set_score, case_set_score*solution_set_score )
            ranked_solutions_by_task[task_id] = sorted(
                flatted_case_set_passed_solutions, key=lambda x: x[1], reverse=True
            )
    ...
    def get_sorted_solutions_without_iter(self):
        logger.info("Start to get sorted solutions without iter")
        ranked_solutions_by_task = defaultdict(list)
        # New dictionary to store test cases information
        solution_test_cases_by_task = defaultdict(dict)
        ...
        for task_id in self.caseset_passed_solutions_by_task.keys():
            ...
            for case_set in self.caseset_passed_solutions_by_task[task_id].keys():
                ...
                test_case_str_set = [
                    self.data_manager.test_case_id_to_string_by_task[task_id][tc]
                    for tc in case_set
                ]

                # Keep original format for ranked_solutions
                flatted_case_set_passed_solutions.append(
                    (solution_str_set, case_set_score * solution_set_score)
                )

                # Store test cases mapping separately
                for solution in solution_str_set:
                    solution_test_cases_by_task[task_id][solution] = {
                        "test_cases": test_case_str_set,
                        "score": case_set_score * solution_set_score,
                    }
        ...
        return ranked_solutions_by_task, solution_test_cases_by_task
